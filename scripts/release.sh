...
if [ -z "${GOLLYX_KLEIN_PELICAN_HOME}" ]; then
...
if [[ -e "${GOLLYX_KLEIN_PELICAN_HOME}/environment.${PROMOTE_DEST_BRANCH}" ]]; then
    source "${GOLLYX_KLEIN_PELICAN_HOME}/environment.${PROMOTE_DEST_BRANCH}"
    make -C "$GOLLYX_KLEIN_PELICAN_HOME" deploy
fi
